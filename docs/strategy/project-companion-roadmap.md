...
| **Design surface** — ~~Design lanes (Open Design, …)~~ **SUPERSEDED** by [native-interactive-surface.md](native-interactive-surface.md): design/preview is Amrita's **native** Surface engine, never an Open Design plugin/dependency. External design-tool *import* (e.g. Figma assets) remains a possible future knowledge connector. | Mockups/options/previews beside the plan, from typed project state | Stage-A deterministic artifact renderers (shipped) → Stage-B sandboxed HTML | No external auth needed for the native surface; any future import connector uses official OAuth + env-ref'd tokens | Artifact states draft→proposed→approved→implemented→verified | Pure renderer/spec tests | Stage-B/C generated-code rendering is approval-gated |
