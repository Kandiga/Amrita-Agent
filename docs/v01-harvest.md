...
| Secret handling (`setSecret`: `ENV_NAME_RE` validation, newline-strip, atomic 0600 write) + redaction | `daemon`/`providers` (file); **`store` `accounts.secret_ref` (done, WO#1.1)** | the `^[A-Z][A-Z0-9_]*$` validation — now also a `store` schema CHECK on `accounts.secret_ref`, plus atomic tmp+rename, 0600, `sk-…abc` redaction shape; their tests | storing secrets in `config.json`/`settings`/the DB (the `settings` key tripwire enforces this) |
| Config versioning (`_version`, stepped migration, `config.json.bak`) | `daemon` config; **`store` `settings` (table done, WO#1.1)** | the stepped-migration pattern and backup-before-write | hand-rolled JSON migration if a schema lib fits better |
| Markdown vault model (TASKS.md / DECISIONS.md / CONTEXT.md / memory) | **`store` `tasks` / `decisions` / `memory_entries` (tables done, WO#1.1)**; projection in WO#1.3 | the task statuses, the append-only decision discipline, per-entry memory budget; v0.1's "curated context pack" sums memory entries | the markdown files *as the source of truth* — v2's truth is the event log + these rows (markdown export can come later) |
| Sectioned setup wizard (`SECTIONS` table, idempotent re-run, never default to a broken provider) | `cli` (Phase 3); **`store` `connectors` / `accounts` (tables done, WO#1.1)** | the section model, idempotency, "recommend a working default" UX; the auto-default test | readline/piped-stdin specifics |
